  ...
  results = foreach(i=n_start:n, .packages=c('rgdal', 'raster')) %dopar% {
    ...
    if (all(is.na(getValues(cc))) == FALSE) {
      ...
      if (all(is.na(getValues(mx))) == FALSE) {
        ...
        extent(r) = ext
        crs(r) = crs(swc)
        writeRaster(r, paste0('./compaction/characterization_factors_ts/', month, '/mx_', col, '_', row, '.tif'),
                    format = 'GTiff', overwrite = TRUE)
        
        r = raster(cf_sdx)
        extent(r) = ext
        crs(r) = crs(swc)
        writeRaster(r, paste0('./compaction/characterization_factors_ts/', month, '/sdx_', col, '_', row, '.tif'),
                    format = 'GTiff', overwrite = TRUE)
